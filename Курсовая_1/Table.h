...
struct list
{
	...
	list(std::string _plate, std::string _label, std::string _model, std::string _color, int _num) {
...
public:
	...
	{
		int sum, temp;
		char ch1 = line[0], ch2 = line[1], ch3 = line[2];
	...
	std::string Print()
	{
		...
		for (int i = 0; i < size1; i++)
		{
			...
			if (status[i] == 1)
				result += std::to_string(items[i].hash) + " " + items[i].plate + " " + items[i].label + " " + items[i].model + " " + items[i].color + "\n";
			else
				result += "-\n";
		}
		result += '\n';
